...
interface GNTJoinLike {
    function bags(address) external returns (address);
...
interface PotLike {
    function pie(address) external returns (uint256);
...
//             }
//             // We want to avoid creating a proxy for a interface address that might not be able to handle proxies, then losing the CDP
//             require(csize == 0, "Dst-is-a-interface");
